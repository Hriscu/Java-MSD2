...
✅ The instructor of an optional course will specify zero or more pairs of type (compulsory course abbr, percentage) with the significance: how important is for students to have better grades at that compulsory course, in order to be assigned to their optional course.  
✅ Example: CO1: {(Math, 100%)}; CO2: {(OOP, 50%), (Java, 50%)}. For each optional course, the students will be ordered based on the weighted average of grades in the compulsory courses taken into account for the optional, in order.  
